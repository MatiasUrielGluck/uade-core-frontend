...
export default function DashboardPage() {
  ...
  const avgIn = Math.round(
    series.reduce((a, p) => a + p.in, 0) / series.length
  );
  const avgProc = Math.round(
    series.reduce((a, p) => a + p.processed, 0) / series.length
  );
  const avgP50 = Math.round(
    series.reduce((a, p) => a + p.p50, 0) / series.length
  );
  const avgP95 = Math.round(
    series.reduce((a, p) => a + p.p95, 0) / series.length
  );
...
<Card variant="outlined" className={`${styles.card} ${styles.toolbar}`}>
  <CardContent className={styles.cardContent} style={{ paddingBlock: 12 }}>
    ...
    >
      ...
      >
        <Typography variant="h6" sx={{ mr: 1 }}>Dashboard</Typography>
      ...
      {/* Acciones: en xs usamos Stack con wrap; en md+ parece un ButtonGroup */}
      <Stack
        direction="row"
        spacing={1}
      ...
      >
        <Button
          variant="outlined" size="small" href="/messages?status=FAILED"
          className={styles.toolbarBtn}
          sx={{ flex: { xs: '1 1 48%', md: '0 0 auto' } }}
        >
          Ver fallas
        </Button>
        <Button
          variant="outlined" size="small" href="/messages?status=DLQ"
          className={styles.toolbarBtn}
          sx={{ flex: { xs: '1 1 48%', md: '0 0 auto' } }}
        >
          Ver DLQ
        ...
        <Button
          variant="outlined" size="small" href="/messages"
          className={styles.toolbarBtn}
        ...
        <Card
          variant="outlined"
          className={`${styles.card} ${styles.kpi1} ${styles.kpi}`}
        >
          <CardContent className={styles.cardContent}>
            <Kpi
              label="Mensajes In"
              value={OV.messagesIn.toLocaleString()}
              hint="últimos 15 min"
              trend={series.map((p) => p.in)}
            />
          </CardContent>
        </Card>
        <Card
          variant="outlined"
          className={`${styles.card} ${styles.kpi2} ${styles.kpi}`}
        >
          <CardContent className={styles.cardContent}>
            <Kpi
              label="Procesados"
              value={OV.messagesProcessed.toLocaleString()}
              hint="últimos 15 min"
              trend={series.map((p) => p.processed)}
            />
          </CardContent>
        </Card>
        <Card
          variant="outlined"
          className={`${styles.card} ${styles.kpi3} ${styles.kpi}`}
        >
          <CardContent className={styles.cardContent}>
            <Kpi
              label="Éxito"
              value={`${Math.round(OV.successRate * 100)}%`}
              hint="últimos 15 min"
            />
          </CardContent>
        </Card>
        <Card
          variant="outlined"
          className={`${styles.card} ${styles.kpi4} ${styles.kpi}`}
        >
          <CardContent className={styles.cardContent}>
            <Kpi
              label="DLQ"
              value={OV.dlqPending}
              hint="pendientes de reproceso"
            />
          </CardContent>
        </Card>
        ...
        >
          ...
          >
            <Typography variant="h6" sx={{ mb: 1 }}>
              Throughput (últimos 15 min)
            </Typography>
            <ReactECharts
              className={styles.chart}
              option={throughputOpt as any}
            />
            <Box
              sx={{
                mt: 1,
        ...
        <Card
          variant="outlined"
          className={`${styles.card} ${styles.errors} ${styles.midCard}`}
        >
          <CardContent
            className={styles.cardContent}
            sx={{ overflow: "visible" }}
          >
            <Typography variant="h6" sx={{ mb: 1 }}>
              Errores por módulo (15 min)
            </Typography>
            <ReactECharts
              className={styles.chart}
              option={errorsByModuleOpt as any}
            />
        ...
        <Card
          variant="outlined"
          className={`${styles.card} ${styles.depth} ${styles.midCard}`}
        >
          <CardContent
            className={styles.cardContent}
            sx={{ overflow: "visible" }}
          >
            <Typography variant="h6" sx={{ mb: 1 }}>
              Queue Depth por tópico
            </Typography>
            <ReactECharts
              className={styles.chart}
              option={queueDepthOpt as any}
            />
          </CardContent>
        </Card>
            ...
            <Box component="table" sx={{ width: "100%", borderSpacing: 0 }}>
              ...
              >
                {DLQ_RECENT.map((r) => (
                  <tr key={r.id}>
                    <td>
                      <Link href={`/messages?id=${r.id}&status=DLQ`}>
                        {r.id}
                      </Link>
                    </td>
                    <td>{r.topic}</td>
